...
/mnt/data/siping/datasets/manu/uav_gmc_median

Key design:
1. Load champion Trial 22 model weights (runs/optuna_median_search/trial_0022/weights/best.pt).
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate Inference Cache for Trial 22 on GMC Median Dataset")
    parser.add_argument(
        "--weights",
        type=str,
...
def main():
    args = parse_args()
    print("=" * 80)
    print("   UAV Tiny Object Detection: Offline Inference Cacher for Trial 22")
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, desc="Trial 22 Inference & Caching"):
