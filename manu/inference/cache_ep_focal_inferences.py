...
"""
Extract lightweight offline inference cache (.pkl) for the high-recall checkpoint:
runs/energy_preserving_focal/exp_ep_focal_5ep/weights/best_recall.pt

Strict Enforcement of Core Rules:
1. Pure sparse point representation (pred_points float16, pred_scores float16, gt_pts float16).
2. Top-k <= 100 per frame with deep sensitivity threshold (conf_thresh=0.02).
3. Zero storage of heavy heatmaps or image tensors.
4. Total cache file size strictly capped under 30MB (loads in ~1s).

Usage:
    python manu/inference/cache_ep_focal_inferences.py \
        --weights runs/energy_preserving_focal/exp_ep_focal_5ep/weights/best_recall.pt \
        --data /mnt/data/siping/datasets/manu/uav_gmc_median/data.yaml \
        --output runs/gmc_eval/uav_ep_focal_best_recall_cache.pkl \
        --device 0
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Cache Inferences for High-Recall EP-Focal Model")
    parser.add_argument(
        "--data",
        type=str,
    ...
    parser.add_argument(
        "--weights",
        type=str,
        default="runs/energy_preserving_focal/exp_ep_focal_5ep/weights/best_recall.pt",
        help="Path to fine-tuned model checkpoint",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="runs/gmc_eval/uav_ep_focal_best_recall_cache.pkl",
    ...
    parser.add_argument("--top-k", type=int, default=100, help="Max candidate peaks per frame (default: 100)")
...
def main():
    args = parse_args()
    print("=" * 80)
    print("   UAV Tiny Object Detection: Offline Inference Cacher for EP-Focal Model")
    ...
    actual_gate = model.p0_highway.gate.item() if hasattr(model, "p0_highway") and hasattr(model.p0_highway.gate, "item") else 0.0
    print(f"[INFO] Loaded State Dict: {matched} layers matched, {skipped} skipped.")
    print(colorstr("cyan", f"[INFO] Active Gate α Value: {actual_gate:.6f}"))
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, desc="EP-Focal Inference & Caching"):
    ...
    print(colorstr("green", f"[SUCCESS] Cache saved successfully: {output_path} ({sz_mb:.2f} MB)"))
