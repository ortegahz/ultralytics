...
"""
Dedicated 4-GPU Parallel Fine-Tuning with PixelShuffle Sub-pixel Upsampling Head (Scheme 3 SOTA Pursuit).

Engineering Rationale (Scheme 3 from Model Evolution Plan):
1. Backbone & Weights: YOLO26HeatmapDetector with upsample_mode="pixelshuffle" (Stride=2, P1 High-Res 320x320 Output).
   Warm-starts directly from the NEW CHAMPION checkpoint:
   `runs/optuna_median_search/trial_0022/weights/best.pt`
   (Matched layers: backbone, neck lateral convs, head convolutions directly transferred;
    PixelShuffle upsamplers initialized with continuous sub-pixel kernels).
...
3. Loss Configuration:
   - Focal Loss with beta=2.40 (Trial 22 negative softening)
   - Offset RegL1Loss (weight=0.45)
   - Optional gentle Soft-IoU loss (default weight=0.10 for safe recall bias)
4. Training Protocol:
...
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune YOLO26 Heatmap with PixelShuffle Sub-Pixel Upsampling")
    parser.add_argument(
        "--data",
        type=str,
    ...
    parser.add_argument("--upsample_mode", type=str, default="pixelshuffle", choices=["pixelshuffle", "nearest"])
    ...
    parser.add_argument("--soft_iou_weight", type=float, default=0.10, help="Soft-IoU loss weight (gentle bias)")
    ...
    parser.add_argument("--project", type=str, default="runs/finetune_pixelshuffle")
    parser.add_argument("--name", type=str, default="exp_trial22_pixelshuffle")
...
def main():
    ...
    print("=" * 100)
    print("   UAV Tiny Object Detection: 4-GPU 6-Epoch Fine-Tuning with PixelShuffle (Scheme 3)")
    ...
    # 3. Model initialization with PixelShuffle and warm-start loading
    ...
    print(colorstr("bold", f"\n[INFO] Initializing YOLO26HeatmapDetector with upsample_mode='{args.upsample_mode}'"))
    ...
    print(colorstr("bold", f"[INFO] Warm-starting compatible layers from Trial 22: {weights_path}"))
    ...
    print("=" * 100)
    print(colorstr("bold", colorstr("green", f"\n[SUCCESS] 6-Epoch PixelShuffle Fine-Tuning Complete!")))
