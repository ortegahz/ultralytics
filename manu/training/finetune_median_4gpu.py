...
"""
Dedicated 4-GPU Parallel 6-Epoch Fine-Tuning for Temporal Median Background Mode.

Architecture & Strategy (Option A Execution):
1. Backbone: YOLO26HeatmapDetector (Stride=2, P1 High-Res 320x320 Heatmap Output).
2. Weight Initialization: Strictly warm-starts from trial_0031 checkpoint (Golden Baseline).
3. Loss: Heatmap Focal Loss + Offset L1 (Unchanged to preserve single-variable causality).
4. Multi-GPU: torch.nn.DataParallel across 4 devices (e.g. 0,1,2,3), batch=32 per GPU (total 128).
5. Epochs & LR: Strictly 6 epochs, lr0=0.0001 with Cosine Annealing, prevents overfitting.
6. Validation: Automatically performs full threshold scanning (0.15~0.60) every epoch at dist<=8.0px,
   saving best_f1.pt, best_recall.pt, and logging detailed results to results.csv.

Usage on Server:
    python manu/training/finetune_median_4gpu.py \
...
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune YOLO26 Heatmap on Aligned Median Dataset")
    parser.add_argument(
        "--data",
        type=str,
    ...
    parser.add_argument("--lr0", type=float, default=0.0001, help="Initial learning rate (gentle adaptation)")
    ...
    parser.add_argument("--project", type=str, default="runs/finetune_median")
    parser.add_argument("--name", type=str, default="exp_6ep_median")
...
def main():
    ...
    print("=" * 100)
    print("   UAV Tiny Object Detection: 4-GPU 6-Epoch Fine-Tuning on Temporal Median Mode")
    ...
    print("=" * 100)
    print(colorstr("bold", colorstr("green", f"\n[SUCCESS] 6-Epoch Fine-Tuning Complete!")))
    print(f"Target Baseline (trial_0031) : F1 = 0.8902 | Recall = 84.64% | Precision = 93.89%")
